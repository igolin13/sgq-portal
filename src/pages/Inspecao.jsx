...
export default function Inspecao() {
  ...
  // Tokens de tema — dark/light
  const t = dark ? {
    card:    '#132030', surface: '#0f1c2a', surface3: '#0c1820',
    text:    '#cce0f0', text3:   '#b0ccdf', muted:    '#6a8a9f', faint: '#4e7090',
    border:  'rgba(255,255,255,0.10)', border3: 'rgba(255,255,255,0.12)',
    border4: 'rgba(255,255,255,0.06)', input:   '#0f1c2a',
    corBg:   'rgba(255,255,255,0.04)',
  } : {
    card:    '#ffffff', surface: '#f3f4f6', surface3: '#f9fafb',
    text:    '#1f2937', text3:   '#374151', muted:    '#6b7280', faint: '#9ca3af',
    border:  '#e5e7eb', border3: '#d1d5db',
    border4: '#f3f4f6', input:   '#ffffff',
    corBg:   null, // usa b.corBg original
  }
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:20, fontFamily:"'DM Sans', sans-serif" }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            ...
            <div>
              <div style={{ fontSize:22, fontWeight:800, color:t.text, lineHeight:1.2 }}>Inspeção</div>
              <div style={{ fontSize:13, color:t.faint, marginTop:2 }}>Litografia · Envernizadeira · Liberação · Fardo Retido</div>
            </div>
          </div>
          <div style={{ padding:'6px 16px', borderRadius:20, background: dark ? 'rgba(59,130,246,0.15)' : '#eff6ff', border: dark ? '1.5px solid rgba(59,130,246,0.3)' : '1.5px solid #bfdbfe', fontSize:12, fontWeight:700, color: dark ? '#60a5fa' : '#1d4ed8', letterSpacing:0.5 }}>
            CQ — Inspeção
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
          {BOTOES.map(b => (
            <button key={b.key} onClick={() => abrirBotao(b.key)}
              style={{ padding:'32px 24px', borderRadius:20, border:`2px solid ${dark ? b.cor+'40' : b.corBorder}`, background: dark ? t.card : b.corBg, cursor:'pointer', textAlign:'left', transition:'all .2s', display:'flex', flexDirection:'column', gap:14 }}
              ...
              <div>
                <div style={{ fontSize:17, fontWeight:700, color:t.text, marginBottom:4 }}>{b.titulo}</div>
  ...
  return (
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16, fontFamily:"'DM Sans', sans-serif", paddingTop: 24 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        ...
        <span style={{ fontSize:14, color:t.faint }}>Inspeção /</span>
        <span style={{ fontSize:14, fontWeight:600, color:t.text3 }}>{btnConf?.titulo}</span>
      </div>

      {/* Abas form/histórico */}
      <div style={{ display:'flex', gap:0, background:t.surface, borderRadius:10, padding:4, width:'fit-content' }}>
          ...
          {abaForm === 'historico' && (
            <div style={{ background:t.card, borderRadius:16, padding:22, display:'flex', flexDirection:'column', gap:14 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
                <span style={{ fontSize:15, fontWeight:700, color:t.text }}>Histórico — {btnConf?.titulo}</span>
                <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
                  <input type="date" value={filtro.inicio} onChange={e => setFiltro(p=>({...p,inicio:e.target.value}))}
                    style={{ padding:'7px 10px', borderRadius:7, border:`1.5px solid ${t.border3}`, fontSize:13, outline:'none', background:t.input, color:t.text }}/>
                  ...
                  <thead>
                    <tr style={{ background:t.surface3 }}>
                      {btnConf?.colunas.map(c => (
                        <th key={c} style={{ padding:'9px 13px', textAlign:'left', fontSize:11, fontWeight:700, color:t.muted, textTransform:'uppercase', letterSpacing:0.4, whiteSpace:'nowrap' }}>{c}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {hist.map((r, i) => (
                      <tr key={i} style={{ borderTop: i ? `1px solid ${t.border4}` : 'none' }}>
                          ...
                          return (
                            <td key={c} style={{ padding:'10px 13px', whiteSpace:'nowrap' }}>
                              {isResult && cfg
                                ...
                                : <span style={{ color:t.text3 }}>{val}</span>
                              }
                            </td>
                          )
                        })}
                    ...
                    {hist.length === 0 && (
                      <tr><td colSpan={btnConf?.colunas.length} style={{ padding:'24px', textAlign:'center', color:t.faint, fontSize:13 }}>Nenhum registro encontrado</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {abaForm === 'form' && (
          <div style={{ background:t.card, borderRadius:16, padding:18, display:'flex', flexDirection:'column', gap:10, position:'sticky', top:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
              ...
              <span style={{ fontSize:13, fontWeight:700, color:t.text3 }}>Últimos 5 registros</span>
            </div>
            {hist.slice(0, 5).map((r, i) => {
              const cfg = COR_RESULTADO[r.resultado]
              return (
                <div key={i} style={{ padding:'11px 13px', borderRadius:10, background:t.surface3, border:'1px solid #f3f4f6', display:'flex', flexDirection:'column', gap:4 }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {r.resultado && cfg
                      ...
                      : r.sit
                        ? <span style={{ fontSize:11, fontWeight:700, color:t.muted, background:t.surface, padding:'2px 7px', borderRadius:5 }}>{r.sit}</span>
                        : null
                    }
                  </div>
                  <div style={{ fontSize:13, fontWeight:500, color:t.text3 }}>
                    {r.maquina || r.inspetor || '—'}{r.op ? ` · OP: ${r.op}` : ''}
                  </div>
                  {r.inspetor && r.maquina && (
                    <div style={{ fontSize:11, color:t.faint }}>{r.inspetor}</div>
                  )}
                </div>
              )
            })}
            {hist.length === 0 && (
              <div style={{ padding:'20px 0', textAlign:'center', fontSize:13, color:t.faint }}>Nenhum registro hoje</div>
            )}
          </div>
        )}
      </div>
