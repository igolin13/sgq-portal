...
const MAQUINAS    = ['Env 01', 'Env 03', 'Env 05', 'Env 06']
...
const FORM_VAZIO = {
  ...
  // Inspeção
  aspectoVisual: '', aderencia: '', mek: '', registroReserva: '',
  // Medições
  temperatura: '', viscosidade: '', camadaDir: '', camadaEsq: '',
  // Controles
  folhaDupla: '', obsFolhaDupla: '',
  sensorVirgem: '', obsSensorVirgem: '',
  barbometro: '', obsBarbometro: '',
  // Final
  gabarito: '', obsFinais: '',
}
...
const OBRIG_INSP  = ['aspectoVisual', 'aderencia', 'mek', 'registroReserva']
const OBRIG_MED   = ['temperatura', 'viscosidade', 'camadaDir', 'camadaEsq']
const OBRIG_CTRL  = ['folhaDupla', 'sensorVirgem', 'barbometro']
...
function Secao({ titulo, cor = '#374151', children }) {
  return (
    <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ padding: '10px 18px', background: cor }}>
...
export default function InspecaoEnvernizadeira({ onSalvo, onCancelar }) {
  ...
  async function salvar() {
    ...
    setStatus('salvando')
    try {
      await salvarItem(SHAREPOINT_CONFIG.listas.inspecaoEnvernizadeira, { ...form, simplificado })
      setStatus('sucesso')
  ...
  if (status === 'sucesso') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '60px 40px', background: '#fff', borderRadius: 16 }}>
        ...
        <div style={{ fontSize: 14, color: '#6b7280' }}>Inspeção Envernizadeira salva com sucesso.</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 16, borderBottom: '1.5px solid #f3f4f6' }}>
        <div style={{ width: 48, height: 48, borderRadius: 12, background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>✨</div>
        <div>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#1f2937' }}>Inspeção Envernizadeira</div>
          <div style={{ fontSize: 13, color: '#9ca3af' }}>Campos com * são obrigatórios</div>
        </div>
      </div>

      {/* DADOS INICIAIS */}
      <Secao titulo="Dados iniciais" cor="#0f6e56">
        <div style={g3}>
      ...
      {!simplificado && form.expediente && (
        <>
          {/* PRODUÇÃO */}
          <Secao titulo="Produção" cor="#1e40af">
            <div style={g3}>
          ...
          <Secao titulo="Inspeção" cor="#185fa5">
            {[
              { f: 'aspectoVisual',  lbl2: 'Aspecto visual',    opts: OPT_INSP_FULL },
              { f: 'aderencia',      lbl2: 'Aderência',          opts: OPT_INSP_FULL },
              { f: 'mek',            lbl2: 'MEK',                opts: OPT_INSP_FULL },
              { f: 'registroReserva',lbl2: 'Registro / Reserva', opts: OPT_INSP_FULL },
            ].map(({ f, lbl2, opts }) => (
              <div key={f}>
                {lbl(lbl2)}
          ...
          {/* CONTROLES */}
          <Secao titulo="Controles" cor="#534ab7">
            {[
              { f: 'folhaDupla',    obsF: 'obsFolhaDupla',    lbl2: 'Folha dupla',            opts: OPT_SENSOR },
              { f: 'sensorVirgem',  obsF: 'obsSensorVirgem',  lbl2: 'Sensor de folha virgem', opts: OPT_SENSOR },
              { f: 'barbometro',    obsF: 'obsBarbometro',    lbl2: 'Barbômetro',             opts: OPT_SENSOR },
              ...
              <div key={f} style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '14px', borderRadius: 10, background: '#fff', border: '1px solid #e2e8f0' }}>
                {lbl(lbl2)}
                <RadioGroup opcoes={opts} valor={form[f]} onChange={v => campo(f, v)} erro={erros[f]}/>
                <input type="text" placeholder={`Observação...`} value={form[obsF]} onChange={e => campo(obsF, e.target.value)}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={salvar} disabled={status === 'salvando'}
          style={{ flex: 2, padding: '13px 0', borderRadius: 10, background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', opacity: status === 'salvando' ? 0.7 : 1 }}>
